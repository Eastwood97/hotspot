  ...
  export default {
    ...
    created() {
      info().then(response => {
        ...
        for (let j = 0; j < hotNumInfoList.length; j++) {
          const index = this.getDaysBetween(hotNumInfoList[j].capture_time2)
        ...
        console.log(cameraCatInfoServiceHoTnumInfoDateNum)
        for (let z = 0; z < cameraCatInfoServiceHoTnumInfoDateNum.length; z++) {
          const index = this.getDaysBetween(cameraCatInfoServiceHoTnumInfoDateNum[z].capture_time2)
          console.log(cameraCatInfoServiceHoTnumInfoDateNum[z].capture_time2)
          Ydata.splice(parseInt(index), 1, cameraCatInfoServiceHoTnumInfoDateNum[z].num)
        }
        this.drawZhe(Ydata)
      })
    },
    methods: {
      getDaysBetween(dateString1) {
        var startDate = new Date(Date.parse(dateString1));
        var endDate = new Date();
        var sDate1 = startDate.getFullYear() + "-" + (startDate.getMonth() + 1) + "-" + startDate.getDate()
        var sDate2 = endDate.getFullYear() + "-" + (endDate.getMonth() + 1) + "-" + endDate.getDate()
        return this.DateDiff(sDate1, sDate2) - 1
      },
      //计算天数差的函数，通用
